        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                ...
                if preview_url:
                    ...
                    if n_songs == 1:
                        await nearest_vectors.insert(playlist_name, user_id, song_name, is_public)
                    else:
                        await nearest_vectors.update_add_song(playlist_name, user_id, song_name)
...
async def song_list_delete(callback, state):
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cursor:
            ...
            await nearest_vectors.update_delete_song(playlist_name, user_id, song_name)
            await conn.commit()
...
async def ask_for_song_id(callback: CallbackQuery, state):
    ...
    try:
        await nearest_vectors.delete_playlist(playlist_name, user_id)
    except IndexError:
        pass
